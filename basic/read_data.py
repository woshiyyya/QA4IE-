...
def load_metadata(config, data_type):
    metadata_path = os.path.join(config.data_dir, config.data_scale, config.data_type, "metadata_{}.json".format(data_type))
...
def read_data(config, data_type, ref, data_filter=None):
    ...
    if data_filter is None:
        ...
        for vals in zip(*values):
            each = {key: val for key, val in zip(keys, vals)}
            mask.append(data_filter(each, shared))
            newvalues.append(each)
        for vals in newvalues:
            for key in keys:
                newdata[key].append(vals[key])
        ...
        if config.data_filter == 'max':
            if config.data_type == 'span':
                ...
                for sample in y:
                    ...
                    temp0 = sample[0][0]
                    for yi in sample:
                        if yi[0] >= config.num_sents_th:
                            flag1 = False
                        if yi[0] != temp0:
                            flag1 = False
                        if yi[1] >= config.sent_size_th:
                            flag1 = False
                    if(flag1):
                        flag = True
                        ylist.append(sample)
...
def update_config(config, data_sets):
    ...
    if (config.mode == 'train') | (config.mode == 'test'):
